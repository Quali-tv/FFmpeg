...
static const char *const opt_name_bitstream_filters[] = {"bsf", "absf", "vbsf",
                                                         NULL};
static const char *const opt_name_codec_tags[] = {"tag", "atag", "vtag", "stag",
                                                  NULL};
...
static const char *const opt_name_min_muxing_queue_size[] = {
    "min_muxing_queue_size", NULL};
...
#define MATCH_PER_STREAM_OPT(name, type, outvar, fmtctx, st)      \
  {                                                               \
    ...
    if (matches > 1) WARN_MULTIPLE_OPT_USAGE(name, type, so, st); \
  }
    ...
    *sync = 0;
    ...
    for (i = 0; i < input_files[sync_file_idx]->nb_streams; i++)
      if (check_stream_specifier(input_files[sync_file_idx]->ctx,
                                 input_files[sync_file_idx]->ctx->streams[i],
                                 sync) == 1) {
        sync_stream_idx = i;
        break;
      }
    ...
    if (input_streams[input_files[sync_file_idx]->ist_index + sync_stream_idx]
            ->user_set_discard == AVDISCARD_ALL) {
      av_log(NULL, AV_LOG_FATAL,
    ...
    if (negative) /* disable some already defined maps */
      ...
      for (i = 0; i < input_files[file_idx]->nb_streams; i++) {
        if (check_stream_specifier(input_files[file_idx]->ctx,
                                   ...
                                   *p == ':' ? p + 1 : p) <= 0)
          continue;
        if (input_streams[input_files[file_idx]->ist_index + i]
                ->user_set_discard == AVDISCARD_ALL) {
          disabled = 1;
          continue;
        }
...
static int opt_map_channel(void *optctx, const char *opt, const char *arg) {
  ...
  if (m->channel_idx < 0 || m->channel_idx >= st->codecpar->channels ||
      input_streams[input_files[m->file_idx]->ist_index + m->stream_idx]
              ->user_set_discard == AVDISCARD_ALL) {
    if (allow_unused) {
...
static int opt_filter_hw_device(void *optctx, const char *opt,
                                const char *arg) {
  if (filter_hw_device) {
...
static int copy_metadata(char *outspec, char *inspec, AVFormatContext *oc,
  ...
  if (!ic) {
    if (type_out == 'g' || !*outspec) o->metadata_global_manual = 1;
    if (type_out == 's' || !*outspec) o->metadata_streams_manual = 1;
    if (type_out == 'c' || !*outspec) o->metadata_chapters_manual = 1;
    return 0;
  }

  if (type_in == 'g' || type_out == 'g') o->metadata_global_manual = 1;
  if (type_in == 's' || type_out == 's') o->metadata_streams_manual = 1;
  if (type_in == 'c' || type_out == 'c') o->metadata_chapters_manual = 1;
...
static int opt_recording_timestamp(void *optctx, const char *opt,
                                   const char *arg) {
  OptionsContext *o = optctx;
  char buf[128];
  ...
  for (i = 0; i < ic->nb_streams; i++) {
    ...
    switch (par->codec_type) {
      case AVMEDIA_TYPE_VIDEO:
        if (!ist->dec) ist->dec = avcodec_find_decoder(par->codec_id);
        ...
        if (hwaccel_device) {
          ...
          if (!ist->hwaccel_device) exit_program(1);
        }
...
static int open_input_file(OptionsContext *o, const char *filename) {
  ...
  stdin_interaction &=
      strncmp(filename, "pipe:", 5) && strcmp(filename, "/dev/stdin");
  ...
  for (i = 0; i < ic->nb_streams; i++) choose_decoder(o, ic, ic->streams[i]);
  ...
  if (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO ||
    ...
    if (!codec_name) {
      ost->st->codecpar->codec_id = av_guess_codec(
          s->oformat, NULL, s->url, NULL, ost->st->codecpar->codec_type);
      ...
      ost->enc =
          find_codec_or_die(codec_name, ost->st->codecpar->codec_type, 1);
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc,
  ...
  if (!(ost = av_mallocz(sizeof(*ost)))) exit_program(1);
  ...
  if (ost->enc) {
    ...
    ost->encoder_opts =
        filter_codec_opts(o->g->codec_opts, ost->enc->id, oc, st, ost->enc);
    ...
    ost->encoder_opts =
        filter_codec_opts(o->g->codec_opts, AV_CODEC_ID_NONE, oc, st, NULL);
  }

  if (o->bitexact) ost->enc_ctx->flags |= AV_CODEC_FLAG_BITEXACT;
  ...
  MATCH_PER_STREAM_OPT(max_muxing_queue_size, i, ost->max_muxing_queue_size, oc,
                       st);
  ...
  if (!ost->muxing_queue) exit_program(1);
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc,
  ...
  if ((frame_rate || max_frame_rate) && video_sync_method == VSYNC_PASSTHROUGH)
    av_log(NULL, AV_LOG_ERROR,
  ...
  if (!ost->stream_copy) {
    ...
    if (intra_only) video_enc->gop_size = 0;
    ...
    if (do_psnr) video_enc->flags |= AV_CODEC_FLAG_PSNR;
    ...
    if (!ost->avfilter) exit_program(1);
  } else {
...
static OutputStream *new_unknown_stream(OptionsContext *o, AVFormatContext *oc,
                                        int source_index) {
  OutputStream *ost;
...
static OutputStream *new_attachment_stream(OptionsContext *o,
                                           ...
                                           int source_index) {
  OutputStream *ost =
      new_output_stream(o, oc, AVMEDIA_TYPE_ATTACHMENT, source_index);
  ost->stream_copy = 1;
  ost->finished = 1;
  return ost;
}
...
static int open_output_file(OptionsContext *o, const char *filename) {
  ...
  of->min_muxing_queue_size = 0;

  if (o->min_muxing_queue_size) {
  ...
  of->ctx = oc;
  if (o->recording_time != INT64_MAX) oc->duration = o->recording_time;
  ...
  if (!o->nb_stream_maps) {
    ...
    if (!o->subtitle_disable &&
      ...
      for (i = 0; i < nb_input_streams; i++)
        if (input_streams[i]->st->codecpar->codec_type ==
          ...
          if (input_streams[i]->user_set_discard == AVDISCARD_ALL) continue;
          if (output_codec)
    ...
    if (!o->data_disable) {
      enum AVCodecID codec_id =
          av_guess_codec(oc->oformat, NULL, filename, NULL, AVMEDIA_TYPE_DATA);
  ...
  for (i = nb_output_streams - oc->nb_streams; i < nb_output_streams;
    ...
    if ((ost->stream_copy || ost->attachment_filename) &&
      ...
      if (av_opt_set(ost->st->codec, "flags", e->value, 0) < 0) exit_program(1);
  }
#endif
      ...
      switch (ost->enc_ctx->codec_type) {
        case AVMEDIA_TYPE_VIDEO:
          ...
          if (ost->enc_ctx->pix_fmt != AV_PIX_FMT_NONE) {
            ...
            count = 0;
            while (ost->enc->pix_fmts[count] != AV_PIX_FMT_NONE) count++;
            f->formats = av_mallocz_array(count + 1, sizeof(*f->formats));
            if (!f->formats) exit_program(1);
            memcpy(f->formats, ost->enc->pix_fmts,
        ...
        case AVMEDIA_TYPE_AUDIO:
          if (ost->enc_ctx->sample_fmt != AV_SAMPLE_FMT_NONE) {
            ...
            count = 0;
            while (ost->enc->sample_fmts[count] != AV_SAMPLE_FMT_NONE) count++;
            f->formats = av_mallocz_array(count + 1, sizeof(*f->formats));
            if (!f->formats) exit_program(1);
          ...
          if (ost->enc_ctx->sample_rate) {
            ...
            count = 0;
            while (ost->enc->supported_samplerates[count]) count++;
            f->sample_rates =
                av_mallocz_array(count + 1, sizeof(*f->sample_rates));
            if (!f->sample_rates) exit_program(1);
          ...
          if (ost->enc_ctx->channels) {
            ...
            count = 0;
            while (ost->enc->channel_layouts[count]) count++;
            f->channel_layouts =
                av_mallocz_array(count + 1, sizeof(*f->channel_layouts));
            if (!f->channel_layouts) exit_program(1);
  ...
  for (i = 0; i < o->nb_program; i++) {
    ...
    while (*p) {
      ...
      if (!strcmp(key, "program_num")) progid = strtol(p2, NULL, 0);
      av_freep(&to_dealloc);
      av_freep(&key);
    }
...
static int opt_default_new(OptionsContext *o, const char *opt,
                           const char *arg) {
  int ret;
...
static int opt_channel_layout(void *optctx, const char *opt, const char *arg) {
  ...
  if (!ac_str) return AVERROR(ENOMEM);
  av_strlcpy(ac_str, "ac", 3);
  if (stream_str) av_strlcat(ac_str, stream_str, ac_str_size);
...
static int opt_filter_complex(void *optctx, const char *opt, const char *arg) {
  ...
  if (!filtergraphs[nb_filtergraphs - 1]->graph_desc) return AVERROR(ENOMEM);
...
static int opt_filter_complex_script(void *optctx, const char *opt,
                                     const char *arg) {
...
void show_help_default(const char *opt, const char *arg) {
  ...
  show_help_options(options,
                    "Print help / information / capabilities:", OPT_EXIT, 0, 0);
...
const OptionDef options[] = {
    ...
    {"max_error_rate",
     HAS_ARG | OPT_FLOAT,
     {&max_error_rate},
     "ratio of decoding errors (0.0: no errors, 1.0: 100% errors) above which "
     "ffmpeg returns an error instead of success.",
     "maximum error rate"},
    {"discard",
    ...
    {"find_stream_info",
     OPT_BOOL | OPT_PERFILE | OPT_INPUT | OPT_EXPERT,
     {&find_stream_info},
    ...
    {"fpsmax",
     ...
     "rate"},
    {"s",
     OPT_VIDEO | HAS_ARG | OPT_SUBTITLE | OPT_STRING | OPT_SPEC | OPT_INPUT |
         OPT_OUTPUT,
    ...
    {"top",
     ...
     ""},
    {"vtag",
     OPT_VIDEO | HAS_ARG | OPT_EXPERT | OPT_PERFILE | OPT_INPUT | OPT_OUTPUT,
    ...
    {"sample_fmt",
     ...
     "format"},
    {"channel_layout",
     OPT_AUDIO | HAS_ARG | OPT_EXPERT | OPT_PERFILE | OPT_INPUT | OPT_OUTPUT,
    ...
    {"vc",
     ...
     {.func_arg = opt_video_channel},
     "deprecated, use -channel",
     "channel"},
    {"tvstd",
     HAS_ARG | OPT_EXPERT | OPT_VIDEO,
     {.func_arg = opt_video_standard},
    ...
    {"min_muxing_queue_size",
     HAS_ARG | OPT_INT | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT,
     {.off = OFFSET(min_muxing_queue_size)},
     "minimum number of packets that will be buffered before muxing",
     "packets"},
