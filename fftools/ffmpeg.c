...
static int sub2video_get_blank_frame(InputStream *ist) {
  ...
  if ((ret = av_frame_get_buffer(frame, 0)) < 0) return ret;
...
static void sub2video_heartbeat(InputStream *ist, int64_t pts) {
  ...
  for (i = 0; i < infile->nb_streams; i++) {
    ...
    if (!ist2->sub2video.frame) continue;
    /* subtitles seem to be usually muxed ahead of other streams;
       if not, subtracting a larger time here is necessary */
    pts2 = av_rescale_q(pts, ist->st->time_base, ist2->st->time_base) - 1;
    /* do not send the heartbeat frame if the subtitle is already ahead */
    if (pts2 <= ist2->sub2video.last_pts) continue;
    ...
    if (nb_reqs) sub2video_push_ref(ist2, pts2);
  }
}
  ...
  for (i = 0; i < nb_filtergraphs; i++) {
    ...
    for (j = 0; j < fg->nb_inputs; j++) {
      ...
      while (av_fifo_size(ifilter->frame_queue)) {
        AVFrame *frame;
        av_fifo_generic_read(ifilter->frame_queue, &frame, sizeof(frame), NULL);
        av_frame_free(&frame);
      }
      ...
      if (ist->sub2video.sub_queue) {
        while (av_fifo_size(ist->sub2video.sub_queue)) {
          AVSubtitle sub;
          av_fifo_generic_read(ist->sub2video.sub_queue, &sub, sizeof(sub),
                               NULL);
          avsubtitle_free(&sub);
        }
...
static void write_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost,
  ...
  if (!of->header_written ||
    ...
    if (!av_fifo_space(ost->muxing_queue)) {
      ...
      av_log(NULL, AV_LOG_ERROR, "Output stream new size %d:%d.\n",
             ost->st->index, new_size);
...
early_exit:
  ...
  if (debug_ts) {
    av_log(NULL, AV_LOG_INFO,
           ...
           float_pts, enc ? enc->time_base.num : -1,
...
static int init_output_stream(OutputStream *ost, AVFrame *frame, char *error,
                              int error_len);
...
static void do_audio_out(OutputFile *of, OutputStream *ost, AVFrame *frame) {
  ...
  if (!check_recording_time(ost)) return;
...
static void do_video_out(OutputFile *of, OutputStream *ost,
  ...
  for (i = 0; i < nb_frames; i++) {
    ...
    if (vstats_filename && frame_size) do_video_stats(ost, frame_size);
  }

  if (!ost->last_frame) ost->last_frame = av_frame_alloc();
...
static void print_report(int is_last_report, int64_t timer_start,
  ...
  if (is_last_report) print_final_stats(total_size);
}
...
static int check_output_constraints(InputStream *ist, OutputStream *ost) {
  ...
  if (ost->source_index != ist_index) return 0;
...
static void do_streamcopy(InputStream *ist, OutputStream *ost,
  ...
  if (f->recording_time != INT64_MAX) {
    ...
    if (f->start_time != AV_NOPTS_VALUE && copy_ts) start_time += f->start_time;
  ...
  if (ost->enc_ctx->codec_type == AVMEDIA_TYPE_VIDEO) ost->sync_opts++;

  if (av_packet_ref(&opkt, pkt) < 0) exit_program(1);
  ...
  if (pkt->dts == AV_NOPTS_VALUE) {
    ...
    if (!duration) duration = ist->dec_ctx->frame_size;
    opkt.dts =
...
int guess_input_channel_layout(InputStream *ist) {
  ...
  if (!dec->channel_layout) {
    ...
    if (dec->channels > ist->guess_layout_max) return 0;
    dec->channel_layout = av_get_default_channel_layout(dec->channels);
    if (!dec->channel_layout) return 0;
...
static void check_decode_result(InputStream *ist, int *got_output, int ret) {
  ...
  if (ret < 0 && exit_on_error) exit_program(1);
...
static int ifilter_send_frame(InputFilter *ifilter, AVFrame *frame) {
  ...
  if (!ifilter->ist->reinit_filters && fg->graph) need_reinit = 0;
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output,
  ...
  if (ret != AVERROR_EOF) check_decode_result(ist, got_output, ret);

  if (!*got_output || ret < 0) return ret;
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output,
  ...
  // skip the packet.
  if (!eof && pkt && pkt->size == 0) return 0;
  ...
  *duration_pts = decoded_frame->pkt_duration;
  ...
  if (ist->framerate.num) best_effort_timestamp = ist->cfr_next_pts++;
  ...
  if (best_effort_timestamp != AV_NOPTS_VALUE) {
    ...
    if (ts != AV_NOPTS_VALUE) ist->next_pts = ist->pts = ts;
  }
...
static int transcode_subtitles(InputStream *ist, AVPacket *pkt, int *got_output,
  ...
  if (ret < 0 || !*got_output) {
    *decode_failed = 1;
    if (!pkt->size) sub2video_flush(ist);
    return ret;
  }
  ...
  if (ist->sub2video.frame) {
    ...
    if (!ist->sub2video.sub_queue) exit_program(1);
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt,
  ...
  while (ist->decoding_needed) {
    ...
    if (ret < 0) {
      ...
      if (!decode_failed || exit_on_error) exit_program(1);
      break;
    }

    if (got_output) ist->got_output = 1;
...
static void print_sdp(void) {
  ...
  for (i = 0; i < nb_output_files; i++) {
    if (!output_files[i]->header_written) return;
  }

  avc = av_malloc_array(nb_output_files, sizeof(*avc));
  if (!avc) exit_program(1);
...
static enum AVPixelFormat get_format(AVCodecContext *s,
  ...
  for (p = pix_fmts; *p != AV_PIX_FMT_NONE; p++) {
    ...
    if (ist->hw_frames_ctx) {
      ...
      if (!s->hw_frames_ctx) return AV_PIX_FMT_NONE;
    }
...
static int init_input_stream(int ist_index, char *error, int error_len) {
  ...
  if (ist->decoding_needed) {
    ...
    if ((ret = avcodec_open2(ist->dec_ctx, codec, &ist->decoder_opts)) < 0) {
      if (ret == AVERROR_EXPERIMENTAL) abort_codec_experimental(codec, 0);
...
static InputStream *get_input_stream(OutputStream *ost) {
  if (ost->source_index >= 0) return input_streams[ost->source_index];
  return NULL;
}
...
static void flush_muxing_queues(OutputFile *of, int try_improve_time_base) {
  ...
  if (of->waiting_for_min_mux_queue &&
      of->total_muxing_queue_size > of->min_muxing_queue_size) {
    of->waiting_for_min_mux_queue = 0;
    av_log(NULL, AV_LOG_INFO,
           "Done with mux waiting! total_muxing_queue_size: %d "
           "min_muxing_queue_size: %d\n",
           of->total_muxing_queue_size, of->min_muxing_queue_size);
  }
  ...
  do {
    ...
    for (int i = 0; i < of->ctx->nb_streams; i++) {
      ...
      const int ost_muxing_queue_size = av_fifo_size(ost->muxing_queue);

      /* try to improve muxing time_base (only possible if nothing has been
       * written yet) */
      if (try_improve_time_base && !ost_muxing_queue_size) {
        ost->mux_timebase = ost->st->time_base;
      }

      if (!of->waiting_for_min_mux_queue && ost_muxing_queue_size) {
        AVPacket pkt;
        av_fifo_generic_read(ost->muxing_queue, &pkt, sizeof(pkt), NULL);

        if (pkt.size <= ost_muxing_queue_size) {
          ost->muxing_queue_data_size -= pkt.size;
...
static int check_init_output_file(OutputFile *of, int file_index) {
  ...
  if (sdp_filename || want_sdp) print_sdp();
...
static int init_output_stream_streamcopy(OutputStream *ost) {
  ...
  if (ost->rotate_overridden) {
    uint8_t *sd = av_stream_new_side_data(ost->st, AV_PKT_DATA_DISPLAYMATRIX,
                                          sizeof(int32_t) * 9);
    if (sd) av_display_rotation_set((int32_t *)sd, -ost->rotate_override_value);
  }
...
static void set_encoder_id(OutputFile *of, OutputStream *ost) {
  ...
  if (av_dict_get(ost->st->metadata, "encoder", NULL, 0)) return;
  ...
  if (!encoder_string) exit_program(1);
...
static int init_output_stream_encode(OutputStream *ost, AVFrame *frame) {
  ...
  if (enc_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
    ...
    if (ist && !ost->frame_rate.num) ost->frame_rate = ist->framerate;
    if (ist && !ost->frame_rate.num) ost->frame_rate = ist->st->r_frame_rate;
...
static int init_output_stream(OutputStream *ost, AVFrame *frame, char *error,
  ...
  if (ost->encoding_needed) {
    ...
    if ((ret = avcodec_open2(ost->enc_ctx, codec, &ost->encoder_opts)) < 0) {
      if (ret == AVERROR_EXPERIMENTAL) abort_codec_experimental(codec, 1);
      snprintf(
          error, error_len,
...
static void report_new_stream(int input_index, AVPacket *pkt) {
  ...
  if (pkt->stream_index < file->nb_streams_warn) return;
...
static int transcode_init(void) {
  ...
  for (i = 0; i < nb_filtergraphs; i++) {
    ...
    for (j = 0; j < fg->nb_outputs; j++) {
      OutputFilter *ofilter = fg->outputs[j];
      if (!ofilter->ost || ofilter->ost->source_index >= 0) continue;
      if (fg->nb_inputs != 1) continue;
      for (k = nb_input_streams - 1; k >= 0; k--)
        if (fg->inputs[0]->ist == input_streams[k]) break;
...
static OutputStream *choose_output(void) {
  ...
  for (i = 0; i < nb_output_streams; i++) {
    ...
    if (!ost->initialized && !ost->inputs_done) return ost;
...
static void free_input_threads(void) {
  ...
  for (i = 0; i < nb_input_files; i++) free_input_thread(i);
}
...
static int init_input_thread(int i) {
  ...
  if (!f->thread_queue_size) return 0;
...
static int get_input_packet(InputFile *f, AVPacket *pkt) {
  if (f->rate_emu) {
    ...
    for (i = 0; i < f->nb_streams; i++) {
      ...
      if (pts > now) return AVERROR(EAGAIN);
    }
  }

#if HAVE_THREADS
  if (f->thread_queue_size) return get_input_packet_mt(f, pkt);
#endif
...
static int got_eagain(void) {
  int i;
  for (i = 0; i < nb_output_streams; i++)
    if (output_streams[i]->unavailable) return 1;
  return 0;
}

static void reset_eagain(void) {
  int i;
  for (i = 0; i < nb_input_files; i++) input_files[i]->eagain = 0;
  for (i = 0; i < nb_output_streams; i++) output_streams[i]->unavailable = 0;
}
...
static int process_input(int file_index) {
  ...
  if (ret < 0 && ifile->loop) {
    ...
    if (thread_ret < 0) return thread_ret;
#endif
    if (ret < 0)
  ...
  if (ist->discard) goto discard_packet;
  ...
  if (debug_ts) {
    av_log(
        ...
        "pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
  ...
  if (ist->nb_packets == 1) {
    for (i = 0; i < ist->st->nb_side_data; i++) {
      ...
      if (src_sd->type == AV_PKT_DATA_DISPLAYMATRIX) continue;

      if (av_packet_get_side_data(&pkt, src_sd->type, NULL)) continue;

      dst_data = av_packet_new_side_data(&pkt, src_sd->type, src_sd->size);
      if (!dst_data) exit_program(1);
  ...
  if (debug_ts) {
    av_log(NULL, AV_LOG_INFO,
           ...
           "pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
           ifile->ist_index + pkt.stream_index,
           av_get_media_type_string(ist->dec_ctx->codec_type),
           av_ts2str(pkt.pts), av_ts2timestr(pkt.pts, &ist->st->time_base),
           av_ts2str(pkt.dts), av_ts2timestr(pkt.dts, &ist->st->time_base),
...
static int transcode(void) {
...
#if HAVE_THREADS
  if ((ret = init_input_threads()) < 0) goto fail;
#endif
  ...
  for (i = 0; i < nb_input_streams; i++) {
    ...
    if (ist->decoding_needed) {
      ...
      if (ist->hwaccel_uninit) ist->hwaccel_uninit(ist->dec_ctx);
    }
  }
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nb_output_files; i++) {
    if (strcmp(output_files[i]->ctx->oformat->name, "rtp")) want_sdp = 0;
  }

  current_time = ti = get_benchmark_time_stamps();
  if (transcode() < 0) exit_program(1);
  if (do_benchmark) {
