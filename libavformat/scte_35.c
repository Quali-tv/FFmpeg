...
static char *get_hls_string(struct scte35_interface *iface,
                            struct scte35_event *event, int out_state,
                            int64_t pos) {
  int ret;
  ...
  if (out_state == EVENT_IN) {
    ...
    if (event) {
      ...
      if (event->duration != AV_NOPTS_VALUE) {
        int64_t dur = ceil(((double)event->duration * iface->timebase.num) /
                           iface->timebase.den);
    ...
    if (event && event->duration != AV_NOPTS_VALUE) {
      ...
      int64_t elapsed_time =
          ceil(((double)pos * iface->timebase.num) / iface->timebase.den) -
          event->out_pts;
      av_bprintf(&iface->avbstr,
                 "#EXT-X-CUE-OUT-CONT:ElapsedTime=%" PRIu64 ",Duration=%" PRIu64
                 ",SCTE35=%s\n",
  ...
  if (ret == 0) {
    av_log(iface->parent, AV_LOG_INFO, "Out of Memory\n");
    return NULL;
  }

  av_log(iface->parent, AV_LOG_INFO, "%s", iface->avbstr.str);
  return iface->avbstr.str;
}

static struct scte35_event *alloc_scte35_event(int id) {
  struct scte35_event *event = av_mallocz(sizeof(struct scte35_event));
...
static void ref_scte35_event(struct scte35_event *event) { event->ref_count++; }
...
static void unlink_scte35_event(struct scte35_interface *iface,
...
static struct scte35_event *get_event_id(struct scte35_interface *iface,
                                         int id) {
...
*/
static int parse_splice_time(struct scte35_interface *iface, const uint8_t *buf,
                             uint64_t *pts, int64_t pts_adjust) {
  GetBitContext gb;
  int ret;
...
static int parse_schedule_cmd(struct scte35_interface *iface,
                              const uint8_t *buf) {
  const uint8_t *sbuf = buf;
  av_log(iface->parent, AV_LOG_INFO, "Schedule cmd\n");
  return buf - sbuf;
}
/**
     @return length of buffer used
 */
static int parse_insert_cmd(struct scte35_interface *iface, const uint8_t *buf,
                            const int len, int64_t pts_adjust,
                            int64_t current_pts) {
  GetBitContext gb;
  int ret;
  const uint8_t *sbuf = buf;
  int program_splice_flag = 0;
  int duration_flag = 0;
  int splice_immediate_flag = 0;
  int component_tag = 0;
  int auto_return = 0;
  uint16_t u_program_id = 0;
  uint8_t avail_num = 0;
  uint8_t avail_expect = 0;
  int inout = 0;
  int event_id = 0;
  struct scte35_event *event;
  char buffer[128];
  ...
  av_log(iface->parent, AV_LOG_INFO, "%s Insert cmd\n", buffer);
  event_id = AV_RB32(buf);
  av_log(iface->parent, AV_LOG_INFO, "event_id  = %x\n", event_id);
  event = get_event_id(iface, event_id);
  buf += 4;
  cancel = *buf & 0x80;
  av_log(iface->parent, AV_LOG_INFO, "splice_event_cancel_indicator  = %d\n",
         cancel);
  buf++;
  ...
  av_log(iface->parent, AV_LOG_INFO, "out_of_network_indicator  = %d\n", inout);
  av_log(iface->parent, AV_LOG_INFO, "program_splice_flag  = %d\n",
         program_splice_flag);
  av_log(iface->parent, AV_LOG_INFO, "duration_flag  = %d\n", duration_flag);
  av_log(iface->parent, AV_LOG_INFO, "splice_immediate_flag  = %d\n",
         splice_immediate_flag);

  if (program_splice_flag && !splice_immediate_flag) {
    if (inout) {
      av_log(iface->parent, AV_LOG_INFO, "pts_adjust  = %lld\n", pts_adjust);
      ret = parse_splice_time(iface, buf, &event->out_pts, pts_adjust);
      av_log(iface->parent, AV_LOG_INFO, "out_pts  = %lld\n", event->out_pts);
      event->out_pts =
          event->out_pts * iface->timebase.num / iface->timebase.den;
      av_log(iface->parent, AV_LOG_INFO, "out_pts  = %lld\n", event->out_pts);
    } else {
  ...
  if (program_splice_flag == 0) {
    int comp_cnt = *buf++;
    int i;
    av_log(iface->parent, AV_LOG_INFO, "component_count  = %d\n", comp_cnt);
    for (i = 0; i < comp_cnt; i++) {
      component_tag = *buf++;
      av_log(iface->parent, AV_LOG_INFO, "component_tag  = %d\n",
             component_tag);
  ...
  if (duration_flag) {
    ...
    av_log(iface->parent, AV_LOG_INFO, "autoreturn  = %d\n", auto_return);
    skip_bits(&gb, 6);
    event->duration = get_bits64(&gb, 33) + pts_adjust;
    buf += 5;
  }
  u_program_id = AV_RB16(buf);
  av_log(iface->parent, AV_LOG_INFO, "u_program_id  = %hd\n", u_program_id);
  buf += 2;
  avail_num = *buf++;
  av_log(iface->parent, AV_LOG_INFO, "avail_num  = %hhd\n", avail_num);
  avail_expect = *buf++;
  av_log(iface->parent, AV_LOG_INFO, "avail_expect  = %hhd\n", avail_expect);

  return buf - sbuf;
}

static int parse_time_signal_cmd(struct scte35_interface *iface,
                                 const uint8_t *buf) {
  const uint8_t *sbuf = buf;
  av_log(iface->parent, AV_LOG_INFO, "Time Signal cmd\n");
  return buf - sbuf;
}

static int parse_bandwidth_reservation_cmd(struct scte35_interface *iface,
                                           const uint8_t *buf) {
  const uint8_t *sbuf = buf;
  av_log(iface->parent, AV_LOG_INFO, "Band Width reservation cmd\n");
  return buf - sbuf;
}
...
int ff_parse_scte35_pkt(struct scte35_interface *iface, const AVPacket *avpkt) {
  ...
  ret = get_bits(&gb, 1);
  if (ret)
    av_log(iface->parent, AV_LOG_INFO,
           "Section indicator should be 0, since MPEG short sections are to be "
           "used.\n");

  /* private indicator */
  ret = get_bits(&gb, 1);
  if (ret) av_log(iface->parent, AV_LOG_INFO, "corrupt packet\n");
  ...
  av_base64_encode(iface->pkt_base64, AV_BASE64_SIZE(section_length + 3), buf,
                   section_length + 3);
  ...
  switch (cmd_type) {
    case SCTE_CMD_NULL:
      av_log(iface->parent, AV_LOG_INFO, "SCTE-35 Ping Received\n");
      break;
    ...
    case SCTE_CMD_INSERT:
      ret =
          parse_insert_cmd(iface, buf + 14, cmd_length, pts_adjust, avpkt->pts);
      break;
...
/*
 * return event, if there is any event whose starting time aka out_pts is less
 * then current pts. This condition also means that event starting time has
 * already been passed. This function will look for event in events list which
 * resides inside iface.
 */
static struct scte35_event *get_event_ciel_out(struct scte35_interface *iface,
                                               uint64_t pts) {
  struct scte35_event *event = iface->event_list;
  while (event) {
    av_log(iface->parent, AV_LOG_INFO,
           "get_event_ciel_out -- running: %d e_pts: %lld pts: %lld!\n",
           event->running, event->out_pts, pts);
 ...
 */
static struct scte35_event *get_event_floor_in(struct scte35_interface *iface,
                                               uint64_t pts) {
  struct scte35_event *event = iface->event_list;
  struct scte35_event *sevent = NULL;
  while (event) {
    av_log(iface->parent, AV_LOG_INFO,
           "get_event_floor_in -- running: %d e_pts: %lld en_pts: %lld pts: "
           "%lld!\n",
           event->running, event->in_pts, event->nearest_in_pts, pts);
    if (event->in_pts != AV_NOPTS_VALUE && event->in_pts < pts &&
        (event->nearest_in_pts == AV_NOPTS_VALUE ||
         pts <= event->nearest_in_pts)) {
...
static void update_video_pts(struct scte35_interface *iface, uint64_t pts) {
  ...
  if (iface->event_state == EVENT_NONE) {
    ...
    if (event) event->running = 1;
  } else {
    event = get_event_floor_in(iface, pts);
  }
  if (event) iface->current_event = event;
}
...
static struct scte35_event *update_event_state(struct scte35_interface *iface) {
  ...
  if (iface->event_state == EVENT_NONE) iface->current_event = NULL;
 ...
 * to API uses
 */
struct scte35_interface *ff_alloc_scte35_parser(void *parent,
                                                AVRational timebase) {
