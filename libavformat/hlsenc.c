...
typedef enum HLSFlags {
  ...
  HLS_SECOND_LEVEL_SEGMENT_INDEX =
      (1 << 8),  // include segment index in segment filenames when
                 // use_localtime  e.g.: %%03d
  HLS_SECOND_LEVEL_SEGMENT_DURATION =
      (1 << 9),  // include segment duration (microsec) in segment filenames
                 // when use_localtime  e.g.: %%09t
  HLS_SECOND_LEVEL_SEGMENT_SIZE =
      (1 << 10),  // include segment size (bytes) in segment filenames when
                  // use_localtime  e.g.: %%014s
  HLS_TEMP_FILE = (1 << 11),
...
static int hlsenc_io_open(AVFormatContext *s, AVIOContext **pb, char *filename,
  ...
  if (!*pb || !http_base_proto || !hls->http_persistent) {
    ...
    if (err < 0) ff_format_io_close(s, pb);
...
static int hlsenc_io_close(AVFormatContext *s, AVIOContext **pb,
                           char *filename) {
  HLSContext *hls = s->priv_data;
  int http_base_proto = filename ? ff_is_http_proto(filename) : 0;
  int ret = 0;
...
static void set_http_options(AVFormatContext *s, AVDictionary **options,
  ...
  if (c->user_agent) av_dict_set(options, "user_agent", c->user_agent, 0);
  if (c->http_persistent) av_dict_set_int(options, "multiple_requests", 1, 0);
  if (c->timeout >= 0) av_dict_set_int(options, "timeout", c->timeout, 0);
  if (c->headers) av_dict_set(options, "headers", c->headers, 0);
}
...
static void write_codec_attr(AVStream *st, VariantStream *vs) {
  ...
  if (st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE) return;
  if (vs->attr_status == CODEC_ATTRIBUTE_WILL_NOT_BE_WRITTEN) return;
  ...
  if (st->codecpar->codec_id == AV_CODEC_ID_H264) {
    ...
    if (st->codecpar->level != FF_LEVEL_UNKNOWN) level = st->codecpar->level;
    ...
    while (data && (data - st->codecpar->extradata + 19) <
      ...
      if (!(data[0] | data[1] | data[2]) && data[3] == 1 &&
        ...
        remain_size =
            st->codecpar->extradata_size - (data - st->codecpar->extradata);
  ...
  if (!av_stristr(vs->codec_attr, attr)) {
    snprintf(vs->codec_attr + codec_strlen,
             sizeof(vs->codec_attr) - codec_strlen, "%s%s",
             codec_strlen ? "," : "", attr);
  }
  return;
...
static int replace_str_data_in_filename(char **s, const char *filename,
                                        char placeholder,
  ...
  if ((ret = av_bprint_finalize(&buf, s)) < 0) return ret;
  return found_count;
}

static int replace_int_data_in_filename(char **s, const char *filename,
                                        char placeholder, int64_t number) {
  const char *p;
  char c;
...
static int hls_delete_file(HLSContext *hls, AVFormatContext *avf,
  ...
  if (hls->method || (proto && !av_strcasecmp(proto, "http"))) {
    ...
    av_dict_free(&opt);
    if (ret < 0) return hls->ignore_io_errors ? 1 : ret;
...
static int hls_delete_old_segments(AVFormatContext *s, HLSContext *hls,
   ...
   * all segment belongs to the same variant, so do it only once before the
   * loop*/
  if (dirname && av_stristr(dirname, "%v")) {
    if (!vs->varname) {
      if (replace_int_data_in_filename(&dirname_repl, dirname, 'v',
                                       segment->var_stream_idx) < 1) {
        ret = AVERROR(EINVAL);
        goto fail;
      }
    } else {
      if (replace_str_data_in_filename(&dirname_repl, dirname, 'v',
                                       vs->varname) < 1) {
        ret = AVERROR(EINVAL);
        goto fail;
      }
  ...
  while (segment) {
    ...
    if (ret = hls_delete_file(hls, vs->avf, path.str, proto)) goto fail;
    ...
    if ((segment->sub_filename[0] != '\0')) {
      ...
      if (ret = hls_delete_file(hls, vs->vtt_avf, path.str, proto)) goto fail;
    }
    av_bprint_clear(&path);
    previous_segment = segment;
    segment = previous_segment->next;
    if (vs->scte_iface)
            vs->scte_iface->unref_scte35_event(&previous_segment->event);
...
static int randomize(uint8_t *buf, int len) {
  ...
  return 0;
#elif CONFIG_OPENSSL
  if (RAND_bytes(buf, len)) return 0;
#else
...
static int do_encrypt(AVFormatContext *s, VariantStream *vs) {
  ...
  AVIOContext *pb;
  uint8_t key[KEYSIZE];
  char *key_basename_source =
      (hls->master_m3u8_url) ? hls->master_m3u8_url : s->url;

  len = strlen(key_basename_source) + 4 + 1;
  hls->key_basename = av_mallocz(len);
  if (!hls->key_basename) return AVERROR(ENOMEM);
...
static int hls_encryption_start(AVFormatContext *s, VariantStream *vs) {
  ...
  if (ret != sizeof(key)) {
    ...
    if (ret >= 0 || ret == AVERROR_EOF) ret = AVERROR(EINVAL);
    return ret;
  }
...
static int hls_mux_init(AVFormatContext *s, VariantStream *vs) {
  ...
  oc->url = av_strdup("");
  if (!oc->url) return AVERROR(ENOMEM);
  ...
  if (vs->vtt_oformat) {
    ret = avformat_alloc_output_context2(&vs->vtt_avf, vs->vtt_oformat, NULL,
                                         NULL);
    if (ret < 0) return ret;
    vtt_oc = vs->vtt_avf;
  ...
  if (hls->segment_type == SEGMENT_TYPE_FMP4 && hls->max_seg_size > 0) {
    if (hls->http_persistent > 0) {
      ...
      av_log(s, AV_LOG_WARNING,
             "http persistent mode is currently unsupported for fragment mp4 "
             "in the HLS muxer.\n");
    }
    if (hls->max_seg_size > 0) {
      av_log(s, AV_LOG_WARNING,
             "Multi-file byterange mode is currently unsupported in the HLS "
             "muxer.\n");
      return AVERROR_PATCHWELCOME;
    }
  }

  if ((ret = avio_open_dyn_buf(&oc->pb)) < 0) return ret;
...
static HLSSegment *find_segment_by_filename(HLSSegment *segment,
                                            const char *filename) {
  while (segment) {
    if (!av_strcasecmp(segment->filename, filename)) return segment;
    segment = segment->next;
  }
...
static int sls_flags_filename_process(struct AVFormatContext *s,
                                      HLSContext *hls, VariantStream *vs,
                                      HLSSegment *en, double duration,
                                      int64_t pos, int64_t size) {
  if ((hls->flags &
       (HLS_SECOND_LEVEL_SEGMENT_SIZE | HLS_SECOND_LEVEL_SEGMENT_DURATION)) &&
    ...
    if (hls->flags & HLS_SECOND_LEVEL_SEGMENT_SIZE) {
      char *filename = NULL;
      if (replace_int_data_in_filename(&filename, vs->avf->url, 's',
                                       pos + size) < 1) {
    ...
    if (hls->flags & HLS_SECOND_LEVEL_SEGMENT_DURATION) {
      ...
      if (replace_int_data_in_filename(
              &filename, vs->avf->url, 't',
              (int64_t)round(duration * HLS_MICROSECOND_UNIT)) < 1) {
        av_log(hls, AV_LOG_ERROR,
               "Invalid second level segment filename template '%s', "
...
static int sls_flag_check_duration_size_index(HLSContext *hls) {
  ...
  if (hls->flags & HLS_SECOND_LEVEL_SEGMENT_DURATION) {
    av_log(hls, AV_LOG_ERROR,
           "second_level_segment_duration hls_flag requires strftime to be "
           "true\n");
    ret = AVERROR(EINVAL);
  }
  if (hls->flags & HLS_SECOND_LEVEL_SEGMENT_SIZE) {
...
static int sls_flag_check_duration_size(HLSContext *hls, VariantStream *vs) {
  ...
  if ((hls->flags & HLS_SECOND_LEVEL_SEGMENT_SIZE) && !segment_renaming_ok) {
    av_log(hls, AV_LOG_ERROR,
           "second_level_segment_size hls_flag works only with file protocol "
           "segment names\n");
    ret = AVERROR(EINVAL);
  }
...
static void sls_flag_file_rename(HLSContext *hls, VariantStream *vs,
                                 char *old_filename) {
  if ((hls->flags &
       (HLS_SECOND_LEVEL_SEGMENT_SIZE | HLS_SECOND_LEVEL_SEGMENT_DURATION)) &&
      strlen(vs->current_segment_final_filename_fmt)) {
...
#if FF_API_HLS_WRAP
            oc->url, 'd',
            c->wrap ? vs->sequence % c->wrap : vs->sequence) < 1) {
#else
...
static int hls_append_segment(struct AVFormatContext *s, HLSContext *hls,
  ...
  en->event    = event;
    ...
    if (vs->scte_iface) {
        en->event_state = vs->scte_iface->event_state;
    }
  ...
  if (hls->max_nb_segments && vs->nb_entries >= hls->max_nb_segments) {
    ...
    if (en && hls->flags & HLS_DELETE_SEGMENTS &&
      ...
      vs->old_segments = en;
      if ((ret = hls_delete_old_segments(s, hls, vs)) < 0) return ret;
    } else
      av_freep(&en);
  } else
    vs->nb_entries++;
...
static int parse_playlist(AVFormatContext *s, const char *url,
  ...
  if ((ret = ffio_open_whitelist(
           &in, url, AVIO_FLAG_READ, &s->interrupt_callback, NULL,
...
static int hls_rename_temp_file(AVFormatContext *s, AVFormatContext *oc) {
  ...
  if (!final_filename) return AVERROR(ENOMEM);
...
static int create_master_playlist(AVFormatContext *s,
  ...
  if (!hls->master_m3u8_created) {
    ...
    for (i = 0; i < hls->nb_varstreams; i++)
      if (!hls->var_streams[i].m3u8_created) return 0;
  } else {
  ...
  snprintf(temp_filename, sizeof(temp_filename),
           use_temp_file ? "%s.tmp" : "%s", hls->master_m3u8_url);
  ...
  for (i = 0; i < hls->nb_varstreams; i++) {
    ...
    if (vs->has_video || vs->has_subtitle || !vs->agroup) continue;
  ...
  for (i = 0; i < hls->nb_varstreams; i++) {
    ...
    if (vs->agroup) {
      for (j = 0; j < hls->nb_varstreams; j++) {
        ...
        if (!temp_vs->has_video && !temp_vs->has_subtitle && temp_vs->agroup &&
            !av_strcasecmp(temp_vs->agroup, vs->agroup)) {
          if (!aud_st) aud_st = temp_vs->streams[0];
    ...
    bandwidth = 0;
    if (vid_st) bandwidth += get_stream_bit_rate(vid_st);
    if (aud_st) bandwidth += get_stream_bit_rate(aud_st);
    ...
    if (vid_st && vs->sgroup) {
      sgroup = vs->sgroup;
      vtt_m3u8_rel_name =
          get_relative_url(hls->master_m3u8_url, vs->vtt_m3u8_name);
      ...
      ff_hls_write_subtitle_rendition(
          hls->m3u8_out, sgroup, vtt_m3u8_rel_name, vs->language, i,
          hls->has_default_key ? vs->is_default : 1);
    }
    ...
    if (!hls->has_default_key || !hls->has_video_m3u8) {
      ...
      if (vid_st) {
        ff_hls_write_stream_info(vid_st, hls->m3u8_out, bandwidth,
                                 m3u8_rel_name, aud_st ? vs->agroup : NULL,
                                 vs->codec_attr, ccgroup, sgroup);
      }
    }
  }
fail:
  if (ret >= 0) hls->master_m3u8_created = 1;
  hlsenc_io_close(s, &hls->m3u8_out, temp_filename);
  if (use_temp_file) ff_rename(temp_filename, hls->master_m3u8_url, s);
...
static int hls_window(AVFormatContext *s, int last, VariantStream *vs) {
  ...
  double *prog_date_time_p =
      (hls->flags & HLS_PROGRAM_DATE_TIME) ? &prog_date_time : NULL;
  int byterange_mode =
      (hls->flags & HLS_SINGLE_FILE) || (hls->max_seg_size > 0);
  ...
  if ((ret = hlsenc_io_open(s, byterange_mode ? &hls->m3u8_out : &vs->out,
    ...
    if (hls->ignore_io_errors) ret = 0;
    goto fail;
  }

  for (en = vs->segments; en; en = en->next) {
    if (target_duration <= en->duration) target_duration = lrint(en->duration);
  }

  vs->discontinuity_set = 0;
  ff_hls_write_playlist_header(
      byterange_mode ? hls->m3u8_out : vs->out, hls->version, hls->allowcache,
  ...
  for (en = vs->segments; en; en = en->next) {
            if (vs->scte_iface && en->event) {
                char *str = vs->scte_iface->get_hls_string(vs->scte_iface, en->event, en->event_state, en->pos);
                avio_printf(vs->out, "%s", str);
            }
    ...
    if ((hls->encrypt || hls->key_info_file) &&
      ...
      if (*en->iv_string)
        avio_printf(byterange_mode ? hls->m3u8_out : vs->out, ",IV=0x%s",
                    en->iv_string);
      avio_printf(byterange_mode ? hls->m3u8_out : vs->out, "\n");
      key_uri = en->key_uri;
    ...
    if ((hls->segment_type == SEGMENT_TYPE_FMP4) && (en == vs->segments)) {
      ff_hls_write_init_file(
          ...
          (hls->flags & HLS_SINGLE_FILE) ? en->filename
                                         : vs->fmp4_init_filename,
          hls->flags & HLS_SINGLE_FILE, vs->init_range_length, 0);
    }

    ret = ff_hls_write_file_entry(
        byterange_mode ? hls->m3u8_out : vs->out, en->discont, byterange_mode,
        en->duration, hls->flags & HLS_ROUND_DURATIONS, en->size, en->pos,
        hls->baseurl, en->filename,
        en->discont_program_date_time ? &en->discont_program_date_time
                                      : prog_date_time_p,
  ...
  if (vs->vtt_m3u8_name) {
    ...
    for (en = vs->segments; en; en = en->next) {
      ret = ff_hls_write_file_entry(
          hls->sub_m3u8_out, 0, byterange_mode, en->duration, 0, en->size,
          en->pos, hls->baseurl, en->sub_filename, NULL, 0, 0, 0);
      if (ret < 0) {
        av_log(s, AV_LOG_WARNING, "ff_hls_write_file_entry get error\n");
      }
    }

    if (last) ff_hls_write_end_list(hls->sub_m3u8_out);
  }

fail:
  av_dict_free(&options);
  ret = hlsenc_io_close(s, byterange_mode ? &hls->m3u8_out : &vs->out,
                        temp_filename);
  if (ret < 0) {
    return ret;
  }
  ...
  if (use_temp_file) {
    ...
    if (vs->vtt_m3u8_name) ff_rename(temp_vtt_filename, vs->vtt_m3u8_name, s);
  }
...
static int hls_start(AVFormatContext *s, VariantStream *vs) {
  ...
  if (c->flags & HLS_SINGLE_FILE) {
    ...
    if (!new_name) return AVERROR(ENOMEM);
    ...
    if (replace_int_data_in_filename(
      ...
      av_log(oc, AV_LOG_ERROR,
             "Invalid segment filename template '%s', you can try to use "
             "-strftime 1 with it\n",
             vs->basename);
      return AVERROR(EINVAL);
    }
    ...
    if (c->use_localtime) {
      ...
      if (c->use_localtime_mkdir) {
        ...
        if (!fn_copy) return AVERROR(ENOMEM);
  ...
  if (c->key_info_file || c->encrypt) {
    ...
    if (!vs->encrypt_started || (c->flags & HLS_PERIODIC_REKEY)) {
      if (c->key_info_file) {
        if ((err = hls_encryption_start(s, vs)) < 0) goto fail;
      } else {
        if (!c->encrypt_started) {
          if ((err = do_encrypt(s, vs)) < 0) goto fail;
          c->encrypt_started = 1;
        }
  ...
  if (c->segment_type != SEGMENT_TYPE_FMP4) {
    ...
    if (c->flags & HLS_SINGLE_FILE) {
      ...
      if (!vs->out_single_file)
        if ((err = hlsenc_io_open(s, &vs->out_single_file, vs->basename,
                                  &options)) < 0) {
          if (c->ignore_io_errors) err = 0;
          goto fail;
        }
...
static const char *get_default_pattern_localtime_fmt(AVFormatContext *s) {
  ...
  // also no %s support on MSVC, which invokes the invalid parameter handler on
  // unsupported format strings, instead of returning an error
  if (hls->segment_type == SEGMENT_TYPE_FMP4) {
...
static int append_postfix(char *name, int name_buf_len, int i) {
  ...
  snprintf(name + strlen(name), name_buf_len - strlen(name), POSTFIX_PATTERN,
           i);

  if (strlen(extension)) av_strlcat(name, extension, name_buf_len);
...
static int validate_name(int nb_vs, const char *fn) {
  ...
  if (!fn) return AVERROR(EINVAL);

  fn_dup = av_strdup(fn);
  if (!fn_dup) return AVERROR(ENOMEM);
  filename = av_basename(fn);
  subdir_name = av_dirname(fn_dup);

  if (nb_vs > 1 && !av_stristr(filename, "%v") &&
      !av_stristr(subdir_name, "%v")) {
...
static int format_name(const char *buf, char **s, int index,
  ...
  if (!orig_buf_dup) return AVERROR(ENOMEM);
...
static int get_nth_codec_stream_index(AVFormatContext *s,
  ...
  if (stream_id < 0 || stream_id > s->nb_streams - 1) return -1;
  cnt = 0;
  for (stream_index = 0; stream_index < s->nb_streams; stream_index++) {
    if (s->streams[stream_index]->codecpar->codec_type != codec_type) continue;
    if (cnt == stream_id) return stream_index;
    cnt++;
  }
  return -1;
}
...
static int parse_variant_stream_mapstring(AVFormatContext *s) {
   ...
   * "a:0,v:0 a:1,v:1"
   * "a:0,agroup:a0,default:1,language:ENG a:1,agroup:a1,default:0 v:0,agroup:a0
   * v:1,agroup:a1" This string specifies how to group the audio, video and
   * subtitle streams into different variant streams. The variant stream groups
  ...
  if (!hls->var_streams) return AVERROR(ENOMEM);
  ...
  while (varstr = av_strtok(p, " \t", &saveptr1)) {
    ...
    if (!vs->streams) return AVERROR(ENOMEM);
...
static int parse_cc_stream_mapstring(AVFormatContext *s) {
  ...
  if (!hls->cc_streams) return AVERROR(ENOMEM);
...
static int update_variant_stream_info(AVFormatContext *s) {
  ...
  if (hls->var_stream_map) {
    ...
    if (!hls->var_streams[0].streams) return AVERROR(ENOMEM);
...
static int hls_write_header(AVFormatContext *s) {
  ...
  for (i = 0; i < hls->nb_varstreams; i++) {
    ...
    for (ts_index = 0,j = 0; j < vs->nb_streams; j++) {
      AVStream *inner_st;
      ...
      if ( outer_st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE) {
        ...
        continue;
            ...
            inner_st = vs->avf->streams[ts_index];
            vs->scte_iface = ff_alloc_scte35_parser(vs, outer_st->time_base);
            continue;
        } else {
            inner_st = vs->avf->streams[ts_index];
            avpriv_set_pts_info(outer_st, inner_st->pts_wrap_bits, inner_st->time_base.num, inner_st->time_base.den);
...
static int hls_write_packet(AVFormatContext *s, AVPacket *pkt) {
  ...
  if (!oc) {
   ...
   if (st->codecpar->codec_id == AV_CODEC_ID_SCTE_35) {
        ret = ff_parse_scte35_pkt(vs->scte_iface, pkt);
        return ret;
    }
  ...
  if (vs->sequence - vs->nb_entries > hls->start_sequence &&
    ...
    int64_t after_init_list_dur =
        (vs->sequence - hls->start_sequence - vs->nb_entries) * hls->time;
  ...
  if (is_ref_pkt) {
    if (vs->end_pts == AV_NOPTS_VALUE) vs->end_pts = pkt->pts;
    if (vs->new_start) {
      vs->new_start = 0;
      vs->duration = (double)(pkt->pts - vs->end_pts) * st->time_base.num /
                     st->time_base.den;
      ...
      if (pkt->duration) {
        ...
        av_log(s, AV_LOG_WARNING,
               "pkt->duration = 0, maybe the hls segment duration will not "
               "precise\n");
        vs->duration = (double)(pkt->pts - vs->end_pts) * st->time_base.num /
                       st->time_base.den;
      }
  ...
  if (vs->scte_iface)
        vs->scte_iface->update_video_pts(vs->scte_iface, pkt->pts * st->time_base.num / st->time_base.den);

  can_split = can_split && (pkt->pts - vs->end_pts > 0);
  if (vs->packets_written && can_split &&
      ((av_compare_ts(pkt->pts - vs->start_pts, st->time_base, end_pts,
                    AV_TIME_BASE_Q) >= 0) ||
         (vs->scte_iface && vs->scte_iface->event_state == EVENT_OUT))) {
    int64_t new_start_pos;
    int byterange_mode =
        (hls->flags & HLS_SINGLE_FILE) || (hls->max_seg_size > 0);
    ...
    if (hls->segment_type == SEGMENT_TYPE_FMP4) {
      if (!vs->init_range_length) {
        ...
        if (range_length <= 0) return AVERROR(EINVAL);
        avio_write(vs->out, vs->init_buffer, range_length);
        if (!hls->resend_init_file) av_freep(&vs->init_buffer);
    ...
    if (hls->flags & HLS_SINGLE_FILE) {
      ...
      if (oc->url[0]) {
        proto = avio_find_protocol_name(oc->url);
        use_temp_file =
            proto && !strcmp(proto, "file") && (hls->flags & HLS_TEMP_FILE);
      }
      ...
      if (use_temp_file) hls_rename_temp_file(s, oc);
    }
    ...
    if (vs->start_pos || hls->segment_type != SEGMENT_TYPE_FMP4) {
      ...
      ret = hls_append_segment(s, hls, vs, cur_duration, vs->start_pos, vs->end_pts, event, vs->size);
      vs->end_pts = pkt->pts;
      vs->duration = 0;
      if (ret < 0) {
    ...
    // if we're building a VOD playlist, skip writing the manifest multiple
    // times, and just wait until the end
    ...
    if (hls->flags & HLS_SINGLE_FILE) {
      ...
      if (hls->key_info_file || hls->encrypt) ret = hls_start(s, vs);
...
static void hls_deinit(AVFormatContext *s) {
  ...
  for (i = 0; i < hls->nb_varstreams; i++) {
    ...
    if (hls->resend_init_file) av_freep(&vs->init_buffer);
    ff_delete_scte35_parser(vs->scte_iface);
...
static int hls_write_trailer(struct AVFormatContext *s) {
  ...
  failed:
    ...
    hls_append_segment(s, hls, vs, vs->duration + vs->dpp, vs->start_pos,
                       vs->end_pts, NULL, vs->size);

    sls_flag_file_rename(hls, vs, old_filename);

    if (vtt_oc) {
      if (vtt_oc->pb) av_write_trailer(vtt_oc);
...
static int hls_init(AVFormatContext *s) {
  ...
  if (hls->use_localtime) {
    ...
    if (hls->flags & HLS_SINGLE_FILE) pattern += 2;
  }
  ...
  if ((hls->start_sequence_source_type ==
    ...
    if (hls->start_sequence_source_type ==
      ...
      char b[15];
      struct tm *p, tmbuf;
      if (!(p = localtime_r(&t, &tmbuf))) return AVERROR(errno);
      if (!strftime(b, sizeof(b), "%Y%m%d%H%M%S", p)) return AVERROR(ENOMEM);
  ...
  for (i = 0; i < hls->nb_varstreams; i++) {
    ...
    if (vs->has_video > 1)
      av_log(s, AV_LOG_WARNING,
             "More than a single video stream present, expect issues decoding "
             "it.\n");
    if (hls->segment_type == SEGMENT_TYPE_FMP4) {
    ...
    if (!vs->oformat) return AVERROR_MUXER_NOT_FOUND;
    ...
    if (hls->segment_type == SEGMENT_TYPE_FMP4) {
      ...
      if (hls->flags & HLS_SINGLE_FILE) {
        ...
        if (!vs->fmp4_init_filename) return AVERROR(ENOMEM);
      } else {
        ...
        if (hls->nb_varstreams > 1) {
          if (av_stristr(vs->fmp4_init_filename, "%v")) {
            av_freep(&vs->fmp4_init_filename);
            ret = format_name(hls->fmp4_init_filename, &vs->fmp4_init_filename,
                              i, vs->varname);
          } else {
            ret = append_postfix(vs->fmp4_init_filename, fmp4_init_filename_len,
                                 i);
          }
          if (ret < 0) return ret;
        }
        ...
        if (p) {
          ...
          *p = '\0';
          vs->base_output_dirname =
              av_asprintf("%s%s", vs->m3u8_name, vs->fmp4_init_filename);
          *p = tmp;
        } else {
          vs->base_output_dirname = av_strdup(vs->fmp4_init_filename);
        }
        if (!vs->base_output_dirname) return AVERROR(ENOMEM);
      }
    }

    ret = hls->use_localtime ? sls_flag_check_duration_size(hls, vs)
                             : sls_flag_check_duration_size_index(hls);
    if (ret < 0) return ret;

    if (vs->has_subtitle) {
      vs->vtt_oformat = av_guess_format("webvtt", NULL, NULL);
      if (!vs->vtt_oformat) return AVERROR_MUXER_NOT_FOUND;

      p = strrchr(vs->m3u8_name, '.');
      if (p) *p = '\0';

      vs->vtt_basename = av_asprintf("%s%s", vs->m3u8_name, vtt_pattern);
      if (!vs->vtt_basename) return AVERROR(ENOMEM);
      ...
      if (hls->subtitle_filename) {
        ...
        if (!vs->vtt_m3u8_name) return AVERROR(ENOMEM);
      }
      if (p) *p = '.';
    }

    if ((ret = hls_mux_init(s, vs)) < 0) return ret;
    ...
    if ((ret = hls_start(s, vs)) < 0) return ret;
    vs->number++;
  }
...
static const AVOption options[] = {
  ...
  {"hls_enc",
   ...
   1,
   E},
  {"hls_enc_key", "hex-coded 16 byte key to encrypt the segments", OFFSET(key),
  ...
  {"use_localtime",
   "set filename expansion with strftime at segment creation(will be "
   "deprecated)",
...
#if FF_API_HLS_USE_LOCALTIME
  {"use_localtime_mkdir",
   "create last directory component in strftime-generated filename(will be "
   "deprecated)",
  ...
  {"epoch_us",
   ...
   0,
   AV_OPT_TYPE_CONST,
   {.i64 = HLS_START_SEQUENCE_AS_MICROSECONDS_SINCE_EPOCH},
   INT_MIN,
   INT_MAX,
   E,
   "start_sequence_source_type"},
