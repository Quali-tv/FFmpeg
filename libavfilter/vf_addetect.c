...
static av_always_inline void update(AdDetectContext *s, AVFrame *frame,
                                    int is_silence, int current_sample,
                                    int sample_rate, AVRational time_base) {
...
#define SILENCE_DETECT(name, type)                                           \
  static void silence_detect_##name(AdDetectContext *s, AVFrame *frame,      \
    ...
    for (i = 0; i < nb_samples; i++, p++)                                    \
      update(s, frame, *p<noise && * p> - noise, i, sample_rate, time_base); \
  }
